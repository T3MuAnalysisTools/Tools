 ...
 void  BackgroundSelector::Configure(){
   ...
   readerA->AddVariable( "var_mindca_iso", &var_mindca_iso );
   readerA->AddVariable( "var_pmin", &var_pmin);
   readerA->AddSpectator("var_tauMass",&var_tauMass);
   readerA->AddSpectator("var_tauMassRes",&var_tauMassRes);
   readerA->AddSpectator("var_Eta_Tau",&var_Eta_Tau);
   ...
   readerB->AddVariable( "var_mindca_iso", &var_mindca_iso );
   readerB->AddVariable( "var_pmin", &var_pmin);
   readerB->AddSpectator("var_tauMass",&var_tauMass);
   readerB->AddSpectator("var_tauMassRes",&var_tauMassRes);
   readerB->AddSpectator("var_Eta_Tau",&var_Eta_Tau);
   ...
   readerC->AddVariable( "var_mindca_iso", &var_mindca_iso );
   readerC->AddVariable( "var_pmin", &var_pmin);
   readerC->AddSpectator("var_tauMass",&var_tauMass);
   readerC->AddSpectator("var_tauMassRes",&var_tauMassRes);
   readerC->AddSpectator("var_Eta_Tau",&var_Eta_Tau);
   ...
   readerBarrel->AddVariable( "var_MuMu_minKFChi2", &var_MuMu_minKFChi2 );
   ...
   readerBarrel->AddVariable( "var_mindca_iso", &var_mindca_iso );
   readerBarrel->AddVariable( "var_pmin", &var_pmin);
   readerBarrel->AddSpectator("var_tauMass",&var_tauMass);
   readerBarrel->AddSpectator("var_tauMassRes",&var_tauMassRes);
   readerBarrel->AddSpectator("var_Eta_Tau",&var_Eta_Tau);
   ...
   readerEndcap->AddVariable( "var_MuMu_minKFChi2", &var_MuMu_minKFChi2 );
   ...
   readerEndcap->AddVariable( "var_mindca_iso", &var_mindca_iso );
   readerEndcap->AddVariable( "var_pmin", &var_pmin);
   readerEndcap->AddSpectator("var_tauMass",&var_tauMass);
   readerEndcap->AddSpectator("var_tauMassRes",&var_tauMassRes);
   readerEndcap->AddSpectator("var_Eta_Tau",&var_Eta_Tau);
   ...
   ThreeMuMassVsFu=HConfig.GetTH2D(Name+"_ThreeMuMassVsFu","ThreeMuMassVsFu",20,1,2,20,1.6,1.8,"3#mu mass, GeV","3#mu + 2#nu mass, GeV");

   MuPairsMassEta=HConfig.GetTH2D(Name+"_MuPairsMassEta","MuPairsMassEta",20,0.2,1.7,20,0.2,1.7,"m_{12}, GeV","m_{23}, GeV");
   MuPairsMassEtaPrime=HConfig.GetTH2D(Name+"_MuPairsMassEtaPrime","MuPairsMassEtaPrime",20,0.2,1.7,20,0.2,1.7,"m_{12}, GeV","m_{23}, GeV");
   M12 = HConfig.GetTH1D(Name+"_M12","M12",20,0.2,1.7,"m_{12}, GeV","Events");
   M23 = HConfig.GetTH1D(Name+"_M23","M23",20,0.2,1.7,"m_{23}, GeV","Events");

   M12_cut = HConfig.GetTH1D(Name+"_M12_cut","M12_cut",20,0.2,1.7,"m_{12}, GeV","Events");
   M23_cut = HConfig.GetTH1D(Name+"_M23_cut","M23_cut",20,0.2,1.7,"m_{23}, GeV","Events");


   //  MuPairsMassB1=HConfig.GetTH2D(Name+"_MuPairsMassB1","MuPairsMassB1",20,0.2,1.7,20,0.2,1.7,"m_{12}, GeV","m_{23}, GeV");
   //  MuPairsMassB2=HConfig.GetTH2D(Name+"_MuPairsMassB2","MuPairsMassB2",20,0.2,1.7,20,0.2,1.7,"m_{12}, GeV","m_{23}, GeV");
   //  MuPairsMassE1=HConfig.GetTH2D(Name+"_MuPairsMassE1","MuPairsMassE1",20,0.2,1.7,20,0.2,1.7,"m_{12}, GeV","m_{23}, GeV");
   //  MuPairsMassE2=HConfig.GetTH2D(Name+"_MuPairsMassE2","MuPairsMassE2",20,0.2,1.7,20,0.2,1.7,"m_{12}, GeV","m_{23}, GeV");
 ...
 void  BackgroundSelector::Store_ExtraDist(){ 
   ...
   Extradist2d.push_back(&ThreeMuMassVsFu);
   Extradist2d.push_back(&MuPairsMassEta);
   Extradist2d.push_back(&MuPairsMassEtaPrime);

   Extradist1d.push_back(&M12);
   Extradist1d.push_back(&M23);

   Extradist1d.push_back(&M12_cut);
   Extradist1d.push_back(&M23_cut);


   //  Extradist2d.push_back(&MuPairsMassB1);
   //  Extradist2d.push_back(&MuPairsMassB2);
   //  Extradist2d.push_back(&MuPairsMassE1);
   //  Extradist2d.push_back(&MuPairsMassE2);
 ...
 void  BackgroundSelector::doEvent(){ 
   ...
   for(int iTrigger=0; iTrigger < Ntp->NHLT(); iTrigger++){
     ...
     //    }	 
     if(HLT.Contains("DoubleMu3_Trk_Tau3mu_v" )  ) HLTOk=Ntp->HLTDecision(iTrigger);
     //     if(Ntp->HLTDecision(iTrigger)) std::cout<<"HLT passed:   "<< Ntp->HLTName(iTrigger)  <<std::endl;
     //    if(id!=1){
     //         if(HLT.Contains("DoubleMu3_Trk_Tau3mu_v") && Ntp->HLTDecision(iTrigger)  ) HLTOk=true;
     //       }
   ...
   for(int il1=0; il1 < Ntp->NL1Seeds(); il1++){
     ...
     //     if( Ntp-> L1Decision(il1)) std::cout<<"l1 passed :  "<< Ntp->L1Name(il1) << std::endl;
   ...
   if(status){
     ...
     TLorentzVector GenEta(0,0,0,0);
     TLorentzVector GenEtaPrime(0,0,0,0);
     TLorentzVector GenMuon(0,0,0,0);
     for(unsigned int j=0; j< Ntp->NMCSignalParticles(); j++){
       std::cout<<Ntp->MCSignalParticle_pdgid(j)<< std::endl;
       
       if(abs(Ntp->MCSignalParticle_pdgid(j)) == 431){
	 ...
	 for(unsigned int i = 0; i < Ntp->MCSignalParticle_Nchilds(j); i++){
	   ...
	   if(TString(PDGInfo::pdgIdToName(Ntp->MCSignalParticle_childpdgid(j,i))).Contains("eta_prime")){
	     etaprime = true;
	     GenEtaPrime = Ntp->MCSignalParticle_child_p4(j,i);
	   }

	   if(TString(PDGInfo::pdgIdToName(Ntp->MCSignalParticle_childpdgid(j,i))).Contains("eta")){
	     etaprime = false;
	     GenEta = Ntp->MCSignalParticle_child_p4(j,i);
	   }

	   if(TString(PDGInfo::pdgIdToName(Ntp->MCSignalParticle_childpdgid(j,i))).Contains("mu+")   or TString(PDGInfo::pdgIdToName(Ntp->MCSignalParticle_childpdgid(j,i))).Contains("mu-")){
	     GenMuon= Ntp->MCSignalParticle_child_p4(j,i);
	   }

	 }
       }

       std::cout<<"Gen   prints  "<< std::endl;
       if(etaprime){
	 GenEtaPrime.Print();
	 GenMuon.Print();
       }
	 ...
	 GenMuon.Print();
       ...
       //       std::cout<< Ntp->MCSignalParticle_pdgid(j) <<"    "  <<Ntp->MCSignalParticle_Nchilds(j)  <<std::endl;
       
       //       if(abs(Ntp->MCSignalParticle_pdgid(j)) == 431){       std::cout<<" " << Ntp->MCSignalParticle_Nchilds(j)  << " N childs " << Ntp->MCSignalParticle_Nchilds(j)  <<std::endl;

       
       //	 for(unsigned int i = 0; i < Ntp->MCSignalParticle_Nchilds(j); j++){
	   //	   std::cout<<"pdgid   "<< 	Ntp->MCSignalParticle_childpdgid(j,i) << std::endl;
       //	 }
       
       //   
       
       //	 std::cout<<"  "<< Ntp->MCSignalParticle_childpdgid(j,i) << std::endl;
       
       //       }
       //      if(abs(Ntp->MCParticle_pdgid(j)) == 431){
       //	std::cout<<" " << Ntp->MCSignalParticle_Nchilds(j)  << std::endl;
       //	for(unsigned int i = 0; i < Ntp->MCSignalParticle_Nchilds(j); j++){
     ...
     //     if(!etaprime)     MuPairsMassEta.at(t).Fill(M_osss1,M_osss2);
     //     if(etaprime)     MuPairsMassEtaPrime.at(t).Fill(M_osss1,M_osss12);
     ...
     std::cout<<"reco prints    os "     << std::endl;


     (MuonOS + MuonSS1).Print();
     (MuonOS + MuonSS2).Print();

     if(!etaprime)     MuPairsMassEta.at(t).Fill((MuonOS + MuonSS1).M(),(MuonOS + MuonSS2).M());
     if(etaprime)     MuPairsMassEtaPrime.at(t).Fill((MuonOS + MuonSS1).M(),(MuonOS + MuonSS2).M());

    //    std::cout<<" os sign   "<< Ntp->Muon_charge(os_mu_idx) << std::endl;
    //    std::cout<<" ss sign   "<< Ntp->Muon_charge(ss1_mu_idx) << std::endl;
    //    std::cout<<" ss sign   "<< Ntp->Muon_charge(ss2_mu_idx) << std::endl;


     M23.at(t).Fill((MuonOS + MuonSS1).M(),1);
     M12.at(t).Fill((MuonOS + MuonSS2).M(),1);
     
     if((MuonOS + MuonSS1).M() > 0.548)    M23_cut.at(t).Fill((MuonOS + MuonSS1).M(),1);
     if((MuonOS + MuonSS2).M() > 0.548)    M12_cut.at(t).Fill((MuonOS + MuonSS2).M(),1);
   ...
   //Category B2
	...
	ThreeMuMassVsFu.at(t).Fill(MuSumLV.M(), TauSumLV.M());

...
MakeMVACategoryII::MakeMVACategoryII(TString Name_, TString id_):
    Selection(Name_,id_),
    tauMinMass_(1.731),
    tauMaxMass_(1.823),
...
MakeMVACategoryII::~MakeMVACategoryII(){
...
void  MakeMVACategoryII::Configure(){
  // Set tree branches
  ...
  TMVA_Tree->Branch("var_TrackMuBDT_Track",&var_TrackMuBDT_Track);
  ...
  for(int i=0; i<NCuts; i++){
    ...
    if(i==L1){
      ...
      TrkIDTrackBDT=HConfig.GetTH1D(Name+"_TrkIDTrackBDT","TrkIDTrackBDT",30,-0.3,0.3,"#pi/tracker #mu BDT",""); 
...
void  MakeMVACategoryII::Store_ExtraDist(){ 
  ...
  Extradist1d.push_back(&TrkIDTrackBDT);
...
void  MakeMVACategoryII::doEvent(){ 
    unsigned int t;
    int id(Ntp->GetMCID());
    bool hlt_pass = false;
	...
	if(fabs(Ntp->Muon_P4(Ntp->SortedPtMuons(Ntp->ThreeMuonIndices(final_idx)).at(imu)).Eta()) < 1.2    )
      ...
      var_TrackMuBDT_Track = reader_trackerMuonId->EvaluateMVA("BDT");
      TrkIDTrackBDT.at(t).Fill(var_TrackMuBDT_Track,1);

      Muon1MVAID.at(t).Fill(Muon1DetID,1);
      Muon2MVAID.at(t).Fill(Muon2DetID,1);
      Muon3MVAID.at(t).Fill(Muon3DetID,1);

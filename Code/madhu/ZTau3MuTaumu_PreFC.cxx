...
void  ZTau3MuTaumu_PreFC::Configure(){
  ...
  for(int i=0; i<NCuts;i++){
    ...
    if(i==Tau_mu_Candidate_p)       cut.at(Tau_mu_Candidate_p)=3.5;
  ...
  Selection_Cut_mu_p_eta_after_noreco=HConfig.GetTH2D(Name+"_Selection_Cut_mu_p_eta_after_noreco","Selection_Cut_mu_p_eta_after_noreco",200,0.0,100.0,100,0,5.0,"#mu p, GeV","#mu |#eta|");
  Selection_Cut_mu_p_eta_after_trigger=HConfig.GetTH2D(Name+"_Selection_Cut_mu_p_eta_after_trigger","Selection_Cut_mu_p_eta_after_trigger",200,0.0,100.0,100,0,5.0,"#mu p, GeV","#mu |#eta|");
...
void  ZTau3MuTaumu_PreFC::doEvent(){ 
  ...
  for(unsigned int imu=0; imu < Ntp->NMuons() && signal_idx!=-1; imu++)
    {
          ...
          if(Whether_decay_found){
                  ...
                  */
                  ...
                  if(pass.at(Tau_mu_Candidate_p)&&pass.at(Tau_mu_Candidate_eta)&&!pass.at(Tau_mu_Candidate_recod)){
                      Selection_Cut_mu_p_eta_after_noreco.at(t).Fill(Muon_LV.Vect().Mag(),abs(Muon_LV.Eta()));
                      //std::cout<<"Reco Muon_LV p: "<< Muon_LV.Vect().Mag() <<" pT: "<< Muon_LV.Pt() << " dRmin: "<< dR4_max << std::endl;
                  }
                  if(L1Ok&&HLTOk){
                    Selection_Cut_mu_p_eta_after_trigger.at(t).Fill(Muon_LV.Vect().Mag(),abs(Muon_LV.Eta()));
                  }

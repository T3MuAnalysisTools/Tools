...
void  TMVASignal::doEvent(){ 
  ...
  if(status){
	 ...
	 unsigned int Muon_index_1 =  Ntp->ThreeMuonIndices(final_index).at(0);
    unsigned int Muon_index_2 =  Ntp->ThreeMuonIndices(final_index).at(1);
    unsigned int Muon_index_3 =  Ntp->ThreeMuonIndices(final_index).at(2);
    ...
    TLorentzVector Muon3LV = Ntp->Muon_P4(Muon_index_3);
	 ...
	 var_vertexKFChi2 = Ntp->Vertex_signal_KF_Chi2(0);
	 var_VertexMu3D0Sig = Ntp->Vertex_d0sig_reco(final_idx,2);
	 var_VertexMu3D0 = Ntp->Vertex_di_reco(final_idx,2);
	 ...
	 TVector3 vec_pv = Ntp->Vertex_MatchedRefitPrimaryVertex(final_idx);
	 TVector3 vec_tau = TauLV.Vect();

	 TVector d_pv_sv = vec_sv - vec_pv;
...
void  TMVASignal::Finish(){
  file= new TFile("TMVAInput.root","recreate");

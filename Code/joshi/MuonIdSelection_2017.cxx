...
MuonIdSelection::MuonIdSelection(TString Name_, TString id_):
    Selection(Name_,id_),
    tauMinMass_(1.73),
    tauMaxMass_(1.82),
...
MuonIdSelection::~MuonIdSelection(){
...
void  MuonIdSelection::Configure(){
    // Set tree branches
  ...
  Mu3_isGlobal=HConfig.GetTH1D(Name+"_Mu3isGlobal","Muon 3 is global",2,-0.5,1.5,"#mu3 is global","Events");
  Selection::ConfigureHistograms(); //do not remove
  HConfig.GetHistoInfo(types,CrossSectionandAcceptance,legend,colour); // do not remove
}

void  MuonIdSelection::Store_ExtraDist(){ 
 /* 
  ...
  Extradist1d.push_back(&Mu3_isGlobal);
...
void  MuonIdSelection::doEvent(){ 
  ...
  for(unsigned int i_idx =0; i_idx < Ntp->NThreeMuons(); i_idx++){
    ...
    if (Ntp->NThreeMuons()>0){
      ...
      //      Ntp->Muon_isTrackerMuon(mu3_pt_idx) && (!Ntp->Muon_isGlobalMuon(mu3_pt_idx)));
...
//    if (Ntp->Vertex_RefitPVisValid(final_idx)==1){
		TMVA_Tree->Fill();
		Mu3_isGlobal.at(t).Fill(threeGlobal);
		 // ----- Fill the histograms ----- 
...
/* 
  ...
  */
    file= new TFile("MuonIdSelectionInput.root","recreate");

...
MVATree_NoVeto::MVATree_NoVeto(TString Name_, TString id_):
    Selection(Name_,id_),
    tauMinMass_(1.731),
    tauMaxMass_(1.823),
    tauMinSideBand_(1.6),
...
void  MVATree_NoVeto::Configure(){
    ...
    TMVA_Tree->Branch("var_MuTau_maxdR",&var_MuTau_maxdR);
	 ...
	 TMVA_Tree->Branch("var_MuTau_maxdR",&var_MuTau_maxdR);
	 TMVA_Tree->Branch("var_phiMass", &var_phiMass);
	 TMVA_Tree->Branch("var_omegaMass", &var_omegaMass);
...
void  MVATree_NoVeto::Store_ExtraDist(){ 
...
void  MVATree_NoVeto::doEvent(){ 
    ...
    pass.at(TriggerMatch) = (value.at(TriggerMatch)  <  cut.at(TriggerMatch));
	 ...
	 pass.at(PhiVeto) = true;
	 pass.at(OmegaVeto) = true;
    ...
    bool status=AnalysisCuts(t,w,wobs);
	 ...
	 if(status){
    //    std::cout<<" mindist   "<< Ntp->Isolation_MinDist(final_idx) << std::endl;
    ...
    var_trk_relPt = Ntp->Isolation_MuMaxRelIso(final_idx);
	 var_omegaMass = value.at(OmegaVeto);
	 var_phiMass = value.at(PhiVeto);
...
/* 
  ...
  */
    file= new TFile("MVATree_NoVetoInput.root","recreate");

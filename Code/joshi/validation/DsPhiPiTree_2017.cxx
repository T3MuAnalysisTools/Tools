   ...
   dsMassMax(2.02)
{
   ...
   file= new TFile("FillDsPhiPiTree.root","recreate");
}
...
void  DsPhiPiTree::Configure(){
   ...
   DsPhiPi_Tree->Branch("pv_cov_xx", pv_cov(0,0));
   DsPhiPi_Tree->Branch("pv_cov_yy", pv_cov(1,1));
   DsPhiPi_Tree->Branch("pv_cov_zz", pv_cov(2,2));
   DsPhiPi_Tree->Branch("pv_cov_xy", pv_cov(0,1));
   DsPhiPi_Tree->Branch("pv_cov_yz", pv_cov(1,2));
   DsPhiPi_Tree->Branch("pv_cov_zx", pv_cov(2,0));
   
   DsPhiPi_Tree->Branch("sv_cov_xx", sv_cov(0,0));
   DsPhiPi_Tree->Branch("sv_cov_yy", sv_cov(1,1));
   DsPhiPi_Tree->Branch("sv_cov_zz", sv_cov(2,2));
   DsPhiPi_Tree->Branch("sv_cov_xy", sv_cov(0,1));
   DsPhiPi_Tree->Branch("sv_cov_yz", sv_cov(1,2));
   DsPhiPi_Tree->Branch("sv_cov_zx", sv_cov(2,0));
...
void  DsPhiPiTree::doEvent(){
   ...
   int id(Ntp->GetMCID());
   if (id==1){
      if(Ntp->WhichEra(2017).Contains("RunB")){ era = 0; }
      if(Ntp->WhichEra(2017).Contains("RunC")){ era = 1; }
      if(Ntp->WhichEra(2017).Contains("RunD")){ era = 2; }
      if(Ntp->WhichEra(2017).Contains("RunE")){ era = 3; }
      if(Ntp->WhichEra(2017).Contains("RunF")){ era = 4; }
   }
   else era=-1;
   ...
   if(status){
      ...
      sv_cov = Ntp->Vertex_Signal_KF_Covariance(final_idx, true);
      pv_cov = Ntp->Vertex_PrimaryVertex_Covariance(final_idx, true);
      ...
      for (unsigned int iMuon=0; iMuon<Ntp->NMuons(); iMuon++){
         ...
         if (tmp_dr<0.03 && (abs(tmp_lv.Pt()-Track_Pt)/tmp_lv.Pt())<0.1) {

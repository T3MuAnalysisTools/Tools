...
void  FillMVATree::Configure(){
  ...
  L1Seed=HConfig.GetTH1D(Name+"_L1Seed","L1Seed",3,-0.5,2.5,"DoubleMu/TripleMu","Events");
...
void  FillMVATree::doEvent(){ 
  ...
  for(unsigned int i_idx =0; i_idx < Ntp->NThreeMuons(); i_idx++){
    ...
    bool status=AnalysisCuts(t,w,wobs);
	 ...
	 if(status){
      //if (!threeGlobal) cout<<"Found a tracker muon"<<endl;
   ...
   double maxMuondR = std::max({Muon1LV.DeltaR(TauLV), Muon2LV.DeltaR(TauLV), Muon3LV.DeltaR(TauLV)});
    ...
    if (Ntp->Vertex_RefitPVisValid(final_idx)==1){
		...
		Mu3_isGlobal.at(t).Fill(threeGlobal);
		 // ----- Fill the histograms ----- 
       if (DoubleMuFired && TripleMuFired) L1Seed.at(t).Fill(2.0,w);
       else if (DoubleMuFired && !TripleMuFired) L1Seed.at(t).Fill(0.0,w);
...
/* 
  ...
  */
    file= new TFile("FillMVATreeInput_woTrigMatch.root","recreate");

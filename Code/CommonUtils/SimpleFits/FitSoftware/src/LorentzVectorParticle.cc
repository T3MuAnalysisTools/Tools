...
double LorentzVectorParticle::Parameter(int i){
  if (i==E) {
    return std::sqrt(std::pow(Particle::Parameter(m), 2.0) + std::pow(Particle::Parameter(px), 2.0) + std::pow(Particle::Parameter(py), 2.0) + std::pow(Particle::Parameter(pz), 2.0));
  }
  if (i==p) {
    return std::sqrt(std::pow(Particle::Parameter(px), 2.0) + std::pow(Particle::Parameter(py), 2.0) + std::pow(Particle::Parameter(pz), 2.0));
  }
  if (i==pt) {
    return std::sqrt(std::pow(Particle::Parameter(px), 2.0) + std::pow(Particle::Parameter(py), 2.0));
  }
...
double LorentzVectorParticle::Covariance(int i, int j) {
	if (i==E && j==E) {
		return std::pow(Parameter(E), -2) * (
				std::pow(Parameter(px), 2) * Particle::Covariance(px, px) + std::pow(Parameter(py), 2) * Particle::Covariance(py, py) + std::pow(Parameter(pz), 2) * Particle::Covariance(pz, pz) + Parameter(m) * Parameter(m) * Particle::Covariance(m, m) +

...
void KSTest () 
{
        ...
        for(int i=0; i<no_cats; i++){
            ...
            if(ks_score_sig_unbinned[i]>0.1 && ks_score_bkg_unbinned[i]>0.2 && ks_score_bkg_restr_unbinned[i]>0.40){
        ...
        TLegend *legend = new TLegend(0.81, 0.55, 0.9, 0.75); // Specify legend position (x1, y1, x2, y2)

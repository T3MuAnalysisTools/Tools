...
from TrainConfigsBvsD import configuration,selection
...
def doTrain(configs,training_cuts,mlist,infname):
    ...
    for train in configs:
        for category_wagon in train.keys():
            ...
            prefix = "MCTrain"
            outfname = str(count)+"_"+prefix+".root"
            ...
            background_bd= input.Get("TreeS_Bd")
            background_bu= input.Get("TreeS_Bu")



            signalWeight_ds  = 1.0;
            backgroundWeight_bd = 1.0;
            backgroundWeight_bu = 1.0;
        
  
 
            dataloader.AddSignalTree    ( signal_ds,     signalWeight_ds       );
            dataloader.AddBackgroundTree( background_bu,    backgroundWeight_bu      );
            dataloader.AddBackgroundTree( background_bd,    backgroundWeight_bd      );
            ...
            mycutSig = TCut( cuts + "MC == 1" )# + categoryCut) 
            mycutBkg = TCut( cutb + "MC == 1" )# + categoryCut)# + "&& (var_tauMass < 1.75 || var_tauMass > 1.81) ") 
            ...
            if "BDT" in mlist:
                factory.BookMethod(dataloader, TMVA.Types.kBDT, "BDT",
                                    "!H:!V:NTrees=850:MinNodeSize=2.5%:MaxDepth=5:BoostType=AdaBoost:AdaBoostBeta=0.5:SeparationType=GiniIndex:nCuts=20" )

...
def makeClassifiers(classifierName, inputFile, outputFile,cat, varset, cut):
   ...
   for line in lines:
      ...
      if _input_tag in line:
         ...
         oFile.write("TCut mycutb = \""+cutb+" MC==0 " + category_cut + "  && var_flightLenSig < 100.  && var_maxMuonsDca < 0.3   && var_MaxVertexPairQuality < 30. &&  var_svpvTauAngle < 0.2 \";\n")
         oFile.write("TCut mycuts = \""+cuts+" MC==1 " + category_cut + "  && var_flightLenSig < 100.  && var_maxMuonsDca < 0.3   && var_MaxVertexPairQuality < 30. &&  var_svpvTauAngle < 0.2\";\n")

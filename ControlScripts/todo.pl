...
if($ARGV[0] eq "--help" || $ARGV[0] eq ""){
    ...
    printf("\n                                                    --CMSSWRel <X_Y_Z>  Configure CMSSW_X_Y_Z; Default: CMSSW_10_6_20  \n\n");
...
if( $ARGV[0] eq "--DCache" ){
  ...
  # system(sprintf(" (eval `scram unsetenv -sh`; gfal-ls davs://cmsio7.rc.ufl.edu:1094$DS/)  >> junk0"));
    system(sprintf(" eval `scram unsetenv -sh`; gfal-ls davs://cmsio6.rc.ufl.edu:1094$DS/ | grep .root >&junk0"));
...
#			system(sprintf("echo \"rm *.output  \" >> $OutputDir/workdir$set/Set_$B/Set_$B.sh"));
			...
			system(sprintf("echo \"log          = Set_$B-Condor_\\\$(cluster)_\\\$(proccess).log  \" >> $OutputDir/workdir$set/Set_$B/Condor_Set_$B")); 
			system(sprintf("echo \"getenv       = True\" >> $OutputDir/workdir$set/Set_$B/Condor_Set_$B"));
			system(sprintf("echo \"+MaxRuntime  = 7200\" >> $OutputDir/workdir$set/Set_$B/Condor_Set_$B"));
			system(sprintf("echo \"notification = Error        \" >> $OutputDir/workdir$set/Set_$B/Condor_Set_$B"));
			system(sprintf("echo \"queue  1 \" >> $OutputDir/workdir$set/Set_$B/Condor_Set_$B"));
...
#uberftp  cmsio.rc.ufl.edu 'cd /cmsuf/data//store/user/cherepan/DoubleMuonLowMass/Prod_07_10_2019_DoubleMuonLowMass__Run2017F-17Nov2017-v1/191007_093213/0000; get DsT3MNtuple_1.root'
	...
	#	    system(sprintf("echo \"gfal-copy gsiftp://cmsio.rc.ufl.edu/cmsuf/data/$file . \"  >> $OutputDir/workdir$set/Set_$B/Set_$B-get.sh"));
 		    system(sprintf("echo \"    \(eval \\\`scram unsetenv -sh\\\`; gfal-copy davs://cmsio6.rc.ufl.edu:1094$DS/$myfiletrunc ./\) \"  >> $OutputDir/workdir$set/Set_$B/Set_$B-get.sh"));

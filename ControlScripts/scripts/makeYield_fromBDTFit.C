...
void makeYield_fromBDTFit () 
{
    ...
    */
    ...
    BDT_Data_Max[0] = 100.0;
    BDT_Data_Max[1] = 400.0;
    BDT_Data_Max[2] = 50.0;
    ...
    for(int i=0; i<3; i++){
      ...
      xFrame_bdt[i]->GetXaxis()->SetRangeUser(0.0,0.9);
      xFrame_bdt[i]->GetYaxis()->SetRangeUser(0.0000001,BDT_Data_Max[i]);
    ...
    */
    ...
    for(int i=0; i<3; i++){
      ...
      xFrame_bdtMC[i]->GetXaxis()->SetRangeUser(0.0,0.9);
      //xFrame_bdtMC[i]->GetYaxis()->SetRangeUser(0.0,BDT_Data_Max[i]);
      //canvas2->GetPad(i+1)->SetLogy();
      xFrame_bdtMC[i]->Draw();
    }
    */
    ...
    */
    ...
    for(int i=0; i<3; i++){
      ...
      cout << "Yield mc "+print_label[i]+" : " << BDT_distributionMC[i]->createIntegral(*BDTOutput_x[i],NormSet(*BDTOutput_x[i]),Range("R_Small"))->getVal() * (bdt_MC[i]->sumEntries("1", "R1")) << " Yield Background "+print_label[i]+" : " << (pdf_integral_restricted[i]/(1-pdf_integral_restricted[i]))*nData[i]*TightBDTCutFraction[i] << endl;
        ...
        Int_t N = 100; double step = (X_max - X_min)/N;
